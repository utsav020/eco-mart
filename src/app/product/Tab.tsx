...
//   const id = (params as { id?: string })?.id;
//   // const { addToCart } = useCart();
//   const [buyNowSummary, setBuyNowSummary] = useState<BuyNowSummary | null>(
//     null
//   );
//   const [product, setProduct] = useState<ProductType | null>(null);
//   const [relatedProducts, setRelatedProducts] = useState<ProductType[]>([]);
...
//   // ⭐ PLACE ORDER API
//   const handlePlaceOrder = async (product: ProductType) => {
...
//     try {
//       const res = await fetch(
//         "https://ekomart-backend.onrender.com/api/cart/place-order",
//         {
//           method: "POST",
//           headers: {
...
//   const handleBuyNowSummary = async (product: ProductType) => {
//     const token = localStorage.getItem("token");
...
//     try {
//       const res = await fetch(
//         "https://ekomart-backend.onrender.com/api/cart/buynow/summary",
//         {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
...
//       const data: BuyNowSummary = await res.json();

//       if (!res.ok) {
//         toast.error("Failed to load buy now summary");
//         return null;
//       }

//       setBuyNowSummary(data);
//       localStorage.setItem("buyNowSummary", JSON.stringify(data));
...
//                 <button
//                   onClick={async () => {
//                     const summary = await handleBuyNowSummary(product);
//                     if (summary) {
//                       await handlePlaceOrder(product);
...
//       {/* BUY NOW SUMMARY */}
//       {buyNowSummary && buyNowSummary.items.length > 0 && (
//         <div className="mt-6 border rounded-lg p-5 bg-gray-50 max-w-md">
//           <h3 className="text-lg font-bold mb-4">Order Summary</h3>

//           {buyNowSummary.items.map((item) => (
//             <div key={item.product_id} className="flex gap-4 mb-4">
//               <img
//                 src={
//                   item.image_url ||
//                   product.image ||
...
//             <span>Total</span>
//             <span className="text-[#077D40]">
//               ₹ {buyNowSummary.totalAmount}
//             </span>
//           </div>
//         </div>
//       )}
